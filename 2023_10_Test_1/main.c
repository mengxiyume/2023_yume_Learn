...
//找到返回下标，找不到返回-1
{
	...
	while (left <= right)
	{
		...
		if (key > arr[middle])
		{
			//查找数值大于区间中间的数值，区间左部收缩
			left = middle + 1;
		}
		else if (key < arr[middle])
		{
			//查找数值小于区间中间的数值，区间右部收缩
			right = middle - 1;
		}
		else
		{
...
#define TEST15_ARR_SIZE 20

void Test_16()
{
	printf("TEST_16_OUT\n");

	int arr[TEST15_ARR_SIZE] = { 0 };
	
	//数组初始化
	printf("下标 数值\n");
	int i = 0;
	for (i = 0; i < TEST15_ARR_SIZE; i++)
	{
		arr[i] = i + 23;
		printf("%4d %4d\n", i, arr[i]);
	}
	...
	//二分查找找到数值nFindValue的下标
	int nSubscript = bin_search(arr, 0, TEST15_ARR_SIZE - 1, nFindValue);
	if (nSubscript >= 0)
	{
		printf("%d 在限定范围中的下标为 %d\n", nFindValue, nSubscript);
	}
	else
	{
		printf("在限定范围内并没有 %d 存在\n", nFindValue);
	}
